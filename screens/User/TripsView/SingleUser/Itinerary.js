...
const Itinerary = () => {
  ...
  const ItineraryArr = useSelector(
    state => state.firestore.ordered[ItineraryData]
  );
  const scheduledData = useSelector(
    state => state.firestore.data[ItineraryData]
  );
  ...
  const toggleAlert = () => {
    Alert.alert("Please Select a Time", "toggle AM or PM to add event", [
      { text: "Ok", onPress: () => console.log("Ok Pressed, Alert Closed") }
    ]);
  };
  const clearValues = () => {
        ...
        <Agenda
          ...
          renderItem={item => (
            <View style={styles.item}>
              <Text style={{ color: "#6a6a6a" }}>{item.time}</Text>
